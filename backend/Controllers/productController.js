...
const fetchProducts=asyncHandler(async(req,res)=>{
    try {
        ...
        console.error(error);
        res.status(400).json({error:"Failed to Show Products"})
    }
})
...
const fetchAllProducts=asyncHandler(async(req,res)=>{
    try {
        ...
        console.error(error)
        res.status(400).json({error:"Internal error Ocuured"})
    }
})
...
const addProductReviews=asyncHandler(async(req,res)=>{
    try {
      ...
      console.error(error)
      res.status(400).json({error:"Internal error Happend"})   
    }
})
...
export {addProduct,updateProductDetails,removeProduct,fetchProducts,fetchProductById,fetchAllProducts,addProductReviews,fetchTopProducts,fetchNewProducts};